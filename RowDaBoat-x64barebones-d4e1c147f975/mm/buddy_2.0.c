...
int main()
{
    FILE *f = fopen("resultados_b2.txt", "w+");
    for (int i = 0; i < 100; i++)
    {
        clock_t begin = clock();
        int *baseAddress = malloc(1024 * 1024 * 16);
        //printf("%p %p %ld\n", baseAddress, (void *)baseAddress + HEADER_SIZE, HEADER_SIZE);
        /*int *v1 = pMalloc(16, baseAddress);
    printf("%p\n",v1);
    int *v2 = pMalloc(314, baseAddress);
    pMalloc(16, baseAddress);
    pMalloc(16, baseAddress);
    pMalloc(16, baseAddress);
    pMalloc(16, baseAddress);*/
        ...
        //printHeader(v1);
        //printHeader(v2);
        /*int *v2 = pMalloc(16, baseAddress);
    printf("%p\n", v2);
    printListLevel(findLevel(16));
